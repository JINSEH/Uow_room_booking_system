...
export const createRoom = (req, res) => {
    ...
    const room = db.prepare('INSERT INTO rooms (name, capacity) VALUES (?, ?)').run(name, capacity);
...
export const updateRoom = (req, res) => {
    const { id } = req.params;
    const { name, capacity } = req.body;
    const room = db.prepare('UPDATE rooms SET name = ?, capacity = ? WHERE id = ?').run(name, capacity, id);
    if (room) {
        res.json(room);
    } else {
        res.status(404).json({ error: 'Room not found' });
